...
def lista_pacientes():
    ...
    conn.close()
    return render_template('lista_pacientes.html', pacientes=pacientes_lista, busca=busca)
...
def prontuario(id):
    ...
    registros = conn.execute(
        "SELECT * FROM prontuario WHERE paciente_id = ?", (id,)
    ).fetchall()
    conn.close()
...
def anamnese(id):
    ...
    anamnese = conn.execute(
        "SELECT * FROM anamneses WHERE paciente_id = ?", (id,)
    ).fetchone()
    conn.close()
...
def salvar_anamnese():
    ...
    existente = conn.execute(
        "SELECT * FROM anamneses WHERE paciente_id = ?", (dados['paciente_id'],)
    ).fetchone()
    ...
    if existente:
        conn.execute('''
            ...
            WHERE paciente_id=?
        ''', (dados['alergias'], dados['medicamentos'], dados['doencas'],
              dados['observacoes'], dados['paciente_id']))
    else:
        conn.execute('''
            INSERT INTO anamneses (paciente_id, alergias, medicamentos, doencas, observacoes)
            VALUES (?, ?, ?, ?, ?)
        ''', (dados['paciente_id'], dados['alergias'], dados['medicamentos'],
              dados['doencas'], dados['observacoes']))
