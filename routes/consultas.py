...
def agenda():
    ...
    lista_consultas = conn.execute(query, params).fetchall()
    dentistas = conn.execute("SELECT * FROM usuarios WHERE tipo='dentista'").fetchall()
    conn.close()

    return render_template('agenda.html', consultas=lista_consultas, dentistas=dentistas)
...
def nova_consulta():
    ...
    dentistas = conn.execute("SELECT * FROM usuarios WHERE tipo = 'dentista'").fetchall()
    conn.close()
    return render_template('nova_consulta.html', pacientes=lista_pacientes, dentistas=dentistas)
...
def salvar_consulta():
    ...
    conn.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (dados['paciente_id'], dados['dentista_id'], dados['data'],
          dados['hora'], dados['procedimento'], 'agendado'))
    conn.commit()
    conn.close()
...
def editar_escala(id):
    conn = get_db_connection()
    escala = conn.execute("SELECT * FROM escala_dentistas WHERE id = ?", (id,)).fetchone()
    dentistas = conn.execute("SELECT * FROM usuarios WHERE tipo = 'dentista'").fetchall()
    conn.close()
...
def nova_consulta_paciente():
    ...
    paciente = conn.execute(
        "SELECT * FROM pacientes WHERE usuario_id = ?", (session['usuario_id'],)
    ).fetchone()
    dentistas = conn.execute("SELECT * FROM usuarios WHERE tipo = 'dentista'").fetchall()
    conn.close()

    return render_template('nova_consulta_paciente.html', paciente=paciente, dentistas=dentistas)
...
def salvar_consulta_paciente():
    ...
    ''', (dados['dentista_id'], dados['data'], dados['hora'])).fetchone()
    ...
    ''', (dados['dentista_id'], obter_dia_semana(dados['data']))).fetchone()
    ...
    conn.execute('''
        INSERT INTO consultas (paciente_id, dentista_id, data, hora, procedimento, status)
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (paciente['id'], dados['dentista_id'], dados['data'],
          dados['hora'], dados['procedimento'], 'agendado'))
    conn.commit()
    conn.close()
...
def verificar_dia():
    ...
    return jsonify({"valido": bool(escala)})
...
def agenda_dentista():
    ...
    conn = get_db_connection()
    lista_consultas = conn.execute('''
        SELECT * FROM consultas
        WHERE dentista_id = ? AND data = ?
    ''', (dentista_id, data)).fetchall()
    conn.close()

    return render_template('agenda_dentista.html', consultas=lista_consultas)
...
def painel_agenda():
    ...
    ''').fetchall()
    conn.close()
    return render_template('painel_agenda.html', consultas=lista_consultas)
...
def atualizar_status(id, status):
    conn = get_db_connection()
    conn.execute("UPDATE consultas SET status = ? WHERE id = ?", (status, id))
    conn.commit()
    conn.close()
    return redirect('/consultas')
...
def mover_consulta():
    ...
    conn = get_db_connection()
    conn.execute('''
        UPDATE consultas SET data = ?, hora = ? WHERE id = ?
